...
import 'package:happer_app/camera/api/api_client.dart';

import 'package:happer_app/creator/api/cart_api.dart';
import 'package:happer_app/creator/ui/creator_tab_screen.dart';
import 'package:happer_app/providers/cart_provider.dart';
import 'package:provider/provider.dart';
import 'package:happer_app/dashboard/screens/game_contest_screen.dart';
import 'package:happer_app/dashboard/screens/image_display_screen.dart';
import 'package:happer_app/dashboard/screens/nogame_screen.dart';
import 'package:happer_app/discover/screen/discover_tab_screen.dart';
...
import 'package:happer_app/utils/search_service.dart'; // Import SearchService
import 'package:happer_app/utils/snackbar.dart';
import 'package:happer_app/webservices/user_service.dart'; // Import UserService
import 'package:happer_app/services/notification_service.dart';
...
class _DashboardScreenState extends State<DashboardScreen>
              ...
              child: TabBar(
                ...
                onTap: (index) {
                  if (index == 1 && AppManager.isLoginAsGuest) {
                    showAppSnackBar('Please Log In to access Discover',
                        isSuccess: false);
  ...
  void _onTabTapped(int index) async {
    if (index == 4) {
      if (AppManager.isLoginAsGuest) {
        showAppSnackBar('Please Log In to access the Cart', isSuccess: false);
        return;
      }
      // Cart button index
      Navigator.push(
        ...
        // Handle checkout process when returning from cart
        // if (mounted) {
        //   context.read<CartProvider>().fetchCartItemCount(); // Refresh cart item count
        // }
        if (value == 'checkout') {
      ...
      // ⭐ WISHLIST
      if (AppManager.isLoginAsGuest) {
        showAppSnackBar('Please Log In to access Wishlist', isSuccess: false);
        return;
      }
      ...
      if (AppManager.isLoginAsGuest) {
        showAppSnackBar('Please Log In to Upload the Image', isSuccess: false);
        return;
      }
      ...
      if (AppManager.isLoginAsGuest) {
        showAppSnackBar('Please Log In First', isSuccess: false);
        return;
      }
  ...
  void _proceedToCheckout() async {
    ...
    if (token == null || token.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please log in to proceed to checkout')),
      );
      return;
    }
  ...
  void _openCameraDirectly() async {
    ...
    if (status.isGranted) {
      ...
      if (token == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Please login to take a selfie')),
        );
        return;
      }
